...
SELECT EMP_ID "사번", EMP_NAME "이름", MANAGER_ID "매니저", SALARY "월급"
FROM EMPLOYEE E1 
WHERE EXISTS (SELECT NULL 
FROM EMPLOYEE E2
WHERE E1.MANAGER_ID LIKE E2.EMP_ID)
AND E1.SALARY > (SELECT AVG(E2.SALARY) FROM EMPLOYEE E2);
...
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, NVL((SALARY + (SALARY * BONUS)) * 12, SALARY * 12)
FROM EMPLOYEE E1
WHERE (E1.JOB_CODE, E1.SALARY) IN (SELECT E2.JOB_CODE, AVG(NVL((E2.SALARY + (E2.SALARY * BONUS)) * 12, E2.SALARY * 12)) 
FROM EMPLOYEE E2 GROUP BY E2.JOB_CODE)
AND SUBSTR(EMP_NO, 8, 1) LIKE 2;


SELECT AVG(NVL((SALARY + (SALARY * BONUS)) * 12, SALARY * 12))
FROM EMPLOYEE E1
