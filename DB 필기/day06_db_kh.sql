...
WHERE SUBSTR(EMP_NO, 8, 1) LIKE 2
      AND  (JOB_CODE, ( NVL( (E1.SALARY + (E1.SALARY * E1.BONUS) ) * 12, E1.SALARY * 12))) IN 
(SELECT JOB_CODE, AVG(NVL( (E1.SALARY + (E1.SALARY * E1.BONUS) ) * 12, E1.SALARY * 12))
    FROM EMPLOYEE E2
    GROUP BY E2.JOB_CODE, JOB_CODE 
    HAVING
        NVL((E1.SALARY + (E1.SALARY * E1.BONUS) ) * 12, E1.SALARY * 12) 
        < AVG(NVL( (E2.SALARY + (E2.SALARY * E2.BONUS) ) * 12, E2.SALARY * 12)))ORDER BY "연봉";
